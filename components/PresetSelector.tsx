  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && filtered.length > 0 && (
        <ul ref={listRef} className="absolute z-20 mt-1 w-full bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl overflow-hidden max-h-64 overflow-y-auto">
          {filtered.map((p, i) => (
            <li key={p.programId}>
              <button
                ...
                className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 text-left transition-colors group ${
                  highlightedIndex === i ? "bg-zinc-800" : "hover:bg-zinc-800/60"
                }`}
              >
                ...
                <span
                  className={`shrink-0 text-xs px-2 py-0.5 rounded border font-medium ${
                    CATEGORY_COLORS[p.category] ?? "bg-zinc-700 text-zinc-400 border-zinc-600"
                  }`}
                >
                  {p.category}
                </span>
